...
import 'package:canvas/custom_icons/leaf.dart';


class LeafBox extends StatefulWidget {
  final int numSaved;
  final double size;
  const LeafBox({required this.numSaved, required this.size, Key? key}) : super(key: key);

  _LeafState createState() => _LeafState(numSaved: this.numSaved, size: this.size);
}
...
class NativeClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    path.quadraticBezierTo(size.width, size.height, size.width, size.height - rdRad);
    ...
    path.quadraticBezierTo(size.width / 4 * 3 + ran, fillHeight + rdRad, size.width / 2, fillHeight);
    random =  Random();
    ran = random.nextDouble() / 4; 
    path.quadraticBezierTo(size.width / 4 + ran, fillHeight - rdRad, 0, fillHeight);
    path.lineTo(0, size.height - rdRad);
